...
/*   Updated: 2025/11/01 16:34:37 by falakus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_rays(t_ray *ray, t_player *player, int column)
{
	...
	ray->ray_calc.ray_dir_x = player->player_pos.dir_x
		+ player->player_pos.plane_x * ray->ray_calc.camera_x;
	ray->ray_calc.ray_dir_y = player->player_pos.dir_y
		+ player->player_pos.plane_y * ray->ray_calc.camera_x;
...
void	step_and_side_dist(t_ray *ray, t_player *player)
{
	if (ray->ray_calc.ray_dir_x < 0)
	{
		ray->ray_calc.step_x = -1;
		ray->ray_calc.side_dist_x = (player->player_pos.pos_x
				- ray->ray_calc.map_x) * ray->ray_calc.delta_dist_x;
	}
	else
	{
		ray->ray_calc.step_x = 1;
		ray->ray_calc.side_dist_x = (ray->ray_calc.map_x + 1.0
				- player->player_pos.pos_x) * ray->ray_calc.delta_dist_x;
	}
	if (ray->ray_calc.ray_dir_y < 0)
	{
		ray->ray_calc.step_y = -1;
		ray->ray_calc.side_dist_y = (player->player_pos.pos_y
				- ray->ray_calc.map_y) * ray->ray_calc.delta_dist_y;
	}
	else
	{
		ray->ray_calc.step_y = 1;
		ray->ray_calc.side_dist_y = (ray->ray_calc.map_y + 1.0
				- player->player_pos.pos_y) * ray->ray_calc.delta_dist_y;
	}
}
...
{
	ray->ray_draw.wall_height
		= (int)(SCREEN_HEIGHT / ray->ray_calc.perp_wall_dist);
	ray->ray_draw.wall_start
		= -ray->ray_draw.wall_height / 2 + SCREEN_HEIGHT / 2;
